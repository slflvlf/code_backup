...
MM0 = M0 + Ma0;
D0 = diag([2.44e6, 4.88e6, 4*3.89e9]);
...
nlobj.Weights.OutputVariables = [250 250 250*57.3^2 0.01 0.01 0.01];
...
[p0_his, v0_his, pd0_his, vd0_his, tau0_his, p0_hat_his, v0_hat_his] = deal(zeros(3, Tsim+1));
[x0_his, x0_hat_his] = deal(zeros(6, Tsim+1));  [u0_his, du_his] = deal(zeros(8, Tsim));
x0_his(:, 1) = [p0; v0];       x0_hat_his(:, 1) = [p0; v0];
[f0_his, a0_his, da0_his, df0_his] = deal(zeros(4, Tsim+1));     
...
for k = 1:Tsim
    ...
    vd0_his(:, k+1) = Yd(1, 4:6)';
    
    onlineData.ref = Yd;
%     [u0,onlineData] = nlmpcmoveCodeGeneration(coreData,x0_hat,u0,onlineData);
%     [u0,onlineData] = nlmpcmoveMEX(coreData, x0, u0, onlineData);

    [u0, nloptions, info] = nlmpcmove(nlobj, x0, u0, Yd, nloptions);

    u0_his(:, k+1) = u0;   f0_his(:, k+1) = u0(1:4);  a0_his(:, k+1) = u0(5:8);
    du_his(:, k) = u0_his(:, k+1) - u0_his(:, k);

%     [pd1, pd2, pd3, pd4] = cableforce_to_position(u0, L0, p0);
...
plot32_3lines(pd0_his, p0_his, p0_hat_his, vd0_his, v0_his, v0_hat_his, "semi motion", {"pd", "p", "p_{hat}", 'vd', 'v', 'v_{hat}'}, 4, true)
