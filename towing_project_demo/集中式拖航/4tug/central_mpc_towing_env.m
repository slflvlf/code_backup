...
L0 = [75, 50; -75, 50; -75, 50; 75, -50];
u0 = [0 0 0 0 deg2rad(0) deg2rad(180) deg2rad(180) deg2rad(0)]';
...
[x0_his, x0_hat_his] = deal(zeros(6, max_step+1));  [u0_his, du_his] = deal(zeros(8, max_step));

[f0_his, a0_his, da0_his, df0_his] = deal(zeros(4, max_step+1));     

u0_his(:, 1) = u0;  f0_his(:, 1) = u0(1:4);   a0_his(:, 1) = u0(5 : 8)';
...
delta10 = [175, 100, deg2rad(0)]';
delta20 = [-175, 100, deg2rad(0)]';
delta30 = [-175, -100, deg2rad(0)]';
delta40 = [175, -100, deg2rad(0)]';
...
nlobj.MV = struct('Min',{0; 0; 0; 0; deg2rad(-45); deg2rad(135); deg2rad(135); deg2rad(-45)},...
...
nlobj.Weights.OutputVariables = [250 250 250*57.3^2 1 1 57.3^2];
% nlobj.Weights.OutputVariables = [250 250 250];

% nlobj.Weights.ManipulatedVariables =[1 1 1 5 5 5];
% nlobj.Weights.ManipulatedVariables =[1 1 1 1 0 0 0 0]*0;
% 
% nlobj.Weights.ManipulatedVariablesRate = [50 50 50 50 300 300 300 300]*0;
...
%     [pd0, dot_pd0, ddot_pd0] = ref_model(T, i, p0_init, Detination);
    ...
    % 二阶reference model
    traj_ref = zeros(Np, nx);
    for j = 1 : Np
        [pd0, dot_pd0, ddot_pd0] = ref_model_2ord(Detination, pd0, dot_pd0, wn, eta, vmax, time_step);
        vd0 = rotate_matrix(pd0)' * dot_pd0;
%         vd0 = zeros(3, 1);
        traj_ref(j, :) = [pd0; vd0]';  
    end
    pd0_his(:, i) = traj_ref(1, 1:3);   vd0_his(:, i) = traj_ref(1, 4:6);

    [u0, nloptions, info] = nlmpcmove(nlobj, x0, u0, traj_ref, nloptions);
    f0 = u0(1:4);   a0 = u0(5:8);
    u0_his(:, i+1) = u0;   f0_his(:, i+1) = u0(1:4);  a0_his(:, i+1) = u0(5:8);
    du_his(:, i) = u0_his(:, i+1) - u0_his(:, i);

%     [pd1, pd2, pd3, pd4, dot_pd1, dot_pd2, dot_pd3, dot_pd4] = get_tug_pd(p0, f0, a0, mooring_param, L0);
...
%         p1_hat, p2_hat, p3_hat, p4_hat, Kcable, cable_length0);
    ...
    tau0 = thrusters_configuration(u0(5:8), L0) * u0(1:4);
    [p0, v0] = ship_dynamic_solver(p0, v0, tau0, MM0, D0);
    x0 = [p0; v0];
    x0_his(:, i) = x0;  
    p0_his(:, i) = p0;  v0_his(:, i) = v0;
end

plot32_2lines(pd0_his, p0_his, vd0_his, v0_his, 'mpc轨迹追踪效果', {'pd0', 'p0', 'vd0', 'v0'}, 1, true);
% plot31_2lines(vd0_his, v0_his, 'mpc轨迹追踪效果', {'vd0', 'v0'}, 2, true)
figure(3)
subplot(2, 2, 1); plot(u0_his(1:4, :)'); legend("f1", 'f2', 'f3', 'f4'); legend boxoff; title('f');
subplot(2, 2, 3); plot(u0_his(5:8, :)'/pi*180); legend("a1", 'a2', 'a3', 'a4'); legend boxoff; title('a');
subplot(2, 2, 2); plot(du_his(1:4, :)'); legend("df1", 'df2', 'df3', 'df4'); legend boxoff; title('df');
subplot(2, 2, 4); plot(du_his(5:8, :)'/pi*180); legend("da1", 'da2', 'da3', 'da4'); legend boxoff; title('da');
...
% plot31_4lines(pd0_his, pd0_ref_his, p0_his, p0_hat_his, '拖航循迹效果', {"pd0", 'pd0_{ref}', 'p0', 'p0_{hat}'}, 6)
% 
% 
% 
% plot31_4lines(tau0_wind_his, tau0_current_his, tau0_wave1_his, tau0_wave2_his, 'semi环境力', {'wind', 'current', 'wave1', 'wave2'}, 7);
% 
% plot31_4lines(tau1_wind_his, tau1_current_his, tau1_wave1_his, tau1_wave2_his, 'tug环境力', {'wind', 'current', 'wave1', 'wave2'}, 8);
% 
% 
% 
% 
