                                  ...
                                  h_struct_T *workingset, i_struct_T *qrmanager)
{
  ...
  if (mWConstr != 0) {
    ...
    if (mWConstr >= nVar) {
      ...
      if (loop_ub >= 0) {
        memcpy(&SD->u1.f2.workspace_data[0], &workspace_data[0],
               (loop_ub + 1) * sizeof(real_T));
      }
      st.site = &ve_emlrtRSI;
      xgemm(nVar, mWConstr, qrmanager->Q.data, qrmanager->ldq,
            SD->u1.f2.workspace_data, workspace_size[0], workspace_data,
            workspace_size[0]);
      st.site = &ve_emlrtRSI;
      ...
      b_xgemm(nVar, mWConstr, qrmanager->Q.data, qrmanager->ldq,
              SD->u1.f2.workspace_data, ldw, workspace_data, ldw);
    }
    st.site = &ve_emlrtRSI;
    if (nVar > 2147483646) {
      b_st.site = &db_emlrtRSI;
    ...
    do {
      ...
      if (ldw <= nVar - 1) {
        ...
        if ((constrViolation_minnormX <= 2.2204460492503131E-16) ||
          ...
          st.site = &ve_emlrtRSI;
          c_xcopy(nVar, workspace_data, xCurrent_data);
        } else {
          st.site = &ve_emlrtRSI;
          d_xcopy(nVar, workspace_data, workspace_size[0] + 1, xCurrent_data);
        }
        exitg1 = 1;
      }
    } while (exitg1 == 0);
