               ...
               f_struct_T *QPObjective, d_struct_T *qpoptions)
{
  ...
  if (*STEP_TYPE != 3) {
    ...
    st.site = &je_emlrtRSI;
    b_xcopy(WorkingSet->nVar, TrialState->xstar.data, tmp_data);
  ...
  do {
    ...
    case 1:
      ...
      if ((TrialState->state <= 0) && (TrialState->state != -6)) {
        *STEP_TYPE = 2;
        ...
        st.site = &je_emlrtRSI;
        b_xcopy(nVar, TrialState->xstar.data, tmp_data);
    ...
    case 2:
      ...
      for (idx = 0; idx < mLB; idx++) {
        ...
        if (-b_tmp_data[WorkingSet->indexLB.data[idx] - 1] > nrmGradInf) {
          if ((WorkingSet->indexLB.data[idx] < 1) ||
              (WorkingSet->indexLB.data[idx] > 85)) {
            emlrtDynamicBoundsCheckR2012b(WorkingSet->indexLB.data[idx], 1, 85,
                                          &ub_emlrtBCI, &st);
          }
    ...
    if (guard1) {
      if (TrialState->state != -6) {
        ...
        for (tmp_size = 0; tmp_size < 85; tmp_size++) {
          iH0 = 85 * tmp_size;
          b_st.site = &vh_emlrtRSI;
          g_xcopy(&b_st, tmp_size, Hessian, iH0 + 1);
          Hessian[tmp_size + 85 * tmp_size] = nrmGradInf;
          iH0 += tmp_size;
          b_st.site = &vh_emlrtRSI;
          g_xcopy(&b_st, 84 - tmp_size, Hessian, iH0 + 2);
        }
      }
    }
  } while (exitg1 == 0);
