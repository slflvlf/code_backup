  ...
  if (guard1) {
    ...
    for (b_i = 0; b_i < 10; b_i++) {
      ...
      if (y) {
        ...
        for (nx = 0; nx < 6; nx++) {
          Ck[nx + 6 * nx] = 1;
          c[ic[nx] - 1] = (runtimedata_OutputMin[b_i + 10 * nx] - e) -
                          X[(b_i + 11 * nx) + 1];
        }
        for (i = 0; i < 6; i++) {
          i1 = ic[i] + 6;
        ...
        for (nx = 0; nx < 6; nx++) {
          for (i = 0; i < 6; i++) {
            Jx[((ic[i] + 120 * nx) + 720 * b_i) - 1] = val[i + 6 * nx];
          }
        }
        for (nx = 0; nx < 6; nx++) {
          for (i = 0; i < 6; i++) {
            ...
            Jx[((i1 + 120 * nx) + 720 * b_i) - 1] = Ck[i + 6 * nx];
          }
        }
        for (i = 0; i < 6; i++) {
          Je[ic[i] - 1] = -1;
    ...
    if (trueCount == 0) {
      ...
      if (trueCount > muIntScalarMax_sint32(nx, b_i)) {
      ...
      tmp_size[1] = 80;
      nx = trueCount * 80;
      memset(&SD->u1.f1.tmp_data[0], 0, nx * sizeof(real_T));
      ...
      for (i = 0; i < 10; i++) {
        for (i1 = 0; i1 < 6; i1++) {
          for (nx = 0; nx < trueCount; nx++) {
            Jx_data[(nx + trueCount * i1) + trueCount * 6 * i] =
                Jx[((b_tmp_data[nx] + 120 * i1) + 720 * i) - 1];
          }
        }
      }
      for (i = 0; i < trueCount; i++) {
      ...
      for (i = 0; i < nx; i++) {
        for (i1 = 0; i1 < 24; i1++) {
          varargin_2_data[i1 + 24 * i] = SD->u1.f1.y_data[i + y_size[0] * i1];
        }
      }
