                                ...
                                const real_T x_data[], int32_T ix0)
{
  ...
  if (obj->probType == 2) {
    ...
    if (obj->Aineq.size[0] != 0) {
      ...
      b_st.site = &mf_emlrtRSI;
      e_xgemv(85, obj->sizes[2], obj->Aineq.data, obj->ldA, x_data, ix0,
      ...
      for (idx = 0; idx < mIneq; idx++) {
        ...
        obj->maxConstrWorkspace.data[idx] -= x_data[(ix0 + idx) + 84];
        i = obj->maxConstrWorkspace.size[0];
        if (idx + 1 > i) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &sc_emlrtBCI, &st);
        }
    ...
    memcpy(&obj->maxConstrWorkspace.data[0], &obj->beq[0],
           60U * sizeof(real_T));
    b_st.site = &mf_emlrtRSI;
    e_xgemv(85, 60, obj->Aeq.data, obj->ldA, x_data, ix0,
            obj->maxConstrWorkspace.data);
    b_st.site = &mf_emlrtRSI;
    for (idx = 0; idx < 60; idx++) {
      obj->maxConstrWorkspace.data[idx] =
          (obj->maxConstrWorkspace.data[idx] -
           x_data[((ix0 + mIneq) + idx) + 84]) +
          x_data[((ix0 + obj->sizes[2]) + idx) + 144];
      v = muDoubleScalarMax(
  ...
  if (obj->probType == 2) {
    ...
    if (obj->Aineq.size[0] != 0) {
      ...
      for (idx = 0; idx < mIneq; idx++) {
        ...
        if (idx + 86 > x_size) {
          emlrtDynamicBoundsCheckR2012b(idx + 86, 1, x_size, &sc_emlrtBCI, &st);
        }
        i = obj->maxConstrWorkspace.size[0];
        if (idx + 1 > i) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &sc_emlrtBCI, &st);
        }
        obj->maxConstrWorkspace.data[idx] -= x_data[idx + 85];
        i = obj->maxConstrWorkspace.size[0];
        if (idx + 1 > i) {
          emlrtDynamicBoundsCheckR2012b(idx + 1, 1, i, &sc_emlrtBCI, &st);
        }
                              ...
                              const real_T x_data[])
{
  ...
  if (obj->probType == 2) {
    ...
    for (idx = 0; idx < 60; idx++) {
      obj->maxConstrWorkspace.data[idx] =
          (obj->maxConstrWorkspace.data[idx] - x_data[(mIneq + idx) + 85]) +
          x_data[(obj->sizes[2] + idx) + 145];
      v = muDoubleScalarMax(
          v, muDoubleScalarAbs(obj->maxConstrWorkspace.data[idx]));
    }
  } else {
