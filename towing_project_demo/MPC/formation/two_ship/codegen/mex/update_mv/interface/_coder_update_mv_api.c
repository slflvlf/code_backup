...
static const mxArray *d_emlrt_marshallOut(const real_T u[48]);
                   ...
                   *plhs[3])
{
  ...
  emxInit_real_T(&st, &u_sol, 2, &vb_emlrtRTEI, true);
  emxInit_real_T(&st, &lesp, 2, &vb_emlrtRTEI, true);
  ...
  plhs[0] = c_emlrt_marshallOut(u_sol);
  emxFree_real_T(&u_sol);
  if (nlhs > 1) {
    plhs[1] = d_emlrt_marshallOut(*p_sol);
  }
