                 ...
                 const real_T lb[265])
{
  ...
  mLambda =
      ...
      192;
  st.site = &bd_emlrtRSI;
  c_xcopy(mLambda, TrialState->lambdasqp.data, TrialState->lambdaStopTest.data);
  st.site = &bd_emlrtRSI;
  ...
  if (!Flags->gradOK) {
    ...
    MeritFunction->nlpComplError = computeComplError(
        ...
        WorkingSet->indexUB.data, WorkingSet->indexUB.size[0],
        WorkingSet->sizes[4], TrialState->lambdaStopTest.data,
        TrialState->lambdaStopTest.size[0], WorkingSet->sizes[0] + 193);
    ...
    if (TrialState->sqpIterations > 1) {
      ...
      nlpComplErrorTmp = computeComplError(
          ...
          WorkingSet->indexUB.data, WorkingSet->indexUB.size[0],
          WorkingSet->sizes[4], TrialState->lambdaStopTestPrev.data,
          TrialState->lambdaStopTestPrev.size[0], WorkingSet->sizes[0] + 193);
      ...
      if (d < muDoubleScalarMax(MeritFunction->nlpDualFeasError,
        ...
        st.site = &bd_emlrtRSI;
        c_xcopy(mLambda, TrialState->lambdaStopTestPrev.data,
                TrialState->lambdaStopTest.data);
      } else {
        st.site = &bd_emlrtRSI;
        c_xcopy(mLambda, TrialState->lambdaStopTest.data,
    ...
    if (isFeasible &&
      ...
      if (isFeasible && (TrialState->sqpFval < -1.0E+20)) {
          ...
          if (dxTooSmall) {
            if (!isFeasible) {
              ...
              if (WorkingSet->nActiveConstr > 0) {
                ...
                g_xcopy(&b_st, nActiveConstr, TrialState->lambda.data);
                b_st.site = &id_emlrtRSI;
                ...
                nActiveConstr = mFixed + 191;
                st.site = &bd_emlrtRSI;
                if ((mFixed <= mFixed + 191) && (mFixed + 191 > 2147483646)) {
        ...
        if (guard1) {
          ...
          } else if (TrialState->FunctionEvaluations >= 26500) {
            Flags->done = true;
            TrialState->sqpExitFlag = 0;
          }
        }
...
void test_exit(const emlrtStack *sp, struct_T *MeritFunction,
               ...
               const real_T lb[265], boolean_T *Flags_gradOK,
