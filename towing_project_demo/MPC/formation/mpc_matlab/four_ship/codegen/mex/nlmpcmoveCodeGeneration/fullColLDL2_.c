                  ...
                  int32_T NColsRemain)
{
  ...
  for (k = 0; k < NColsRemain; k++) {
    ...
    if ((LD_diagOffset < 1) || (LD_diagOffset > j)) {
      emlrtDynamicBoundsCheckR2012b(LD_diagOffset, 1, j, &ve_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    ...
    for (idx = 0; idx <= j; idx++) {
      if ((idx + 1 < 1) || (idx + 1 > 1)) {
        emlrtDynamicBoundsCheckR2012b(idx + 1, 1, 1, &we_emlrtBCI,
                                      (emlrtCTX)sp);
      }
      jA = obj->FMat.size[0] * obj->FMat.size[1];
      b = (LD_diagOffset + idx) + 1;
      if ((b < 1) || (b > jA)) {
        emlrtDynamicBoundsCheckR2012b(b, 1, jA, &ve_emlrtBCI, (emlrtCTX)sp);
      }
    ...
    for (idx = 0; idx < subMatrixDim; idx++) {
      ...
      if ((jA < 1) || (jA > j)) {
        emlrtDynamicBoundsCheckR2012b(jA, 1, j, &ve_emlrtBCI, (emlrtCTX)sp);
      }
      j = obj->FMat.size[0] * obj->FMat.size[1];
      if (LD_diagOffset > j) {
        emlrtDynamicBoundsCheckR2012b(LD_diagOffset, 1, j, &ve_emlrtBCI,
