...
void update_mv(const emlrtStack *sp, struct0_T mv[64], const real_T subset_data[],
               const int32_T subset_size[1], struct1_T *param, emxArray_real_T
               *u_sol, real_T p_sol[64], emxArray_real_T *lesp)
{
  real_T ell;
  real_T Nloop;
  int32_T i;
  ...
  /* --------------------------------------------------------- */
  ell = (param->Nev - 1.0) / (256.0 * param->Niter);
  if (ell < 0.0) {
  ...
  for (c_i = 0; c_i < i; c_i++) {
    for (sig = 0; sig < loop_ub; sig++) {
      ...
      for (b_i = 0; b_i < 64; b_i++) {
        param->p[b_i] = lesp->data[b_i + 64 * ((int32_T)ind - 1)];
      ...
      if (((int32_T)ell < 1) || ((int32_T)ell > 64)) {
        emlrtDynamicBoundsCheckR2012b((int32_T)ell, 1, 64, &g_emlrtBCI, sp);
      }
      ...
      b_i = (int32_T)ind - 1;
      mv[(int32_T)ell - 1].p = lesp->data[((int32_T)ell + 64 * b_i) - 1];
      st.site = &emlrtRSI;
      ...
      for (i1 = 0; i1 < 64; i1++) {
        c_lesp[i1] = lesp->data[i1 + 64 * b_i];
      }

      for (b_i = 0; b_i < 64; b_i++) {
        lesp->data[b_i + 64 * (int32_T)ind] = c_lesp[b_i];
      }
      ...
      lesp->data[((int32_T)ell + 64 * (b_i - 1)) - 1] = mv[(int32_T)ell - 1].p;
  ...
  for (i = 0; i < 64; i++) {
    p_sol[i] = lesp->data[i + 64 * (lesp->size[1] - 1)];
  }

  loop_ub = lesp->size[1];
  st.site = &b_emlrtRSI;
  user_control_profile(&st, *(real_T (*)[64])&lesp->data[64 * (loop_ub - 1)],
