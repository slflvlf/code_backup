...
static emlrtRTEInfo ab_emlrtRTEI = { 15,/* lineNo */
  9,                                   /* colNo */
  "one_step",                          /* fName */
  "C:\\\\Users\\\\jiang\\\\Desktop\\\\towing_project_demo\\\\\xe8\xae\xa1\xe7\xae\x97\xe7\xbb\x84\xe4\xbb\xb6\\\\pdfmpc\\\\one_step.m"/* pName */
};
                 ...
                 *tt, emxArray_real_T *xx, emxArray_real_T *uu)
{
  ...
  emxInit_real_T(sp, &k1, 1, &w_emlrtRTEI, true);
  emxInit_real_T(sp, &k2, 1, &x_emlrtRTEI, true);
  emxInit_real_T(sp, &k3, 1, &y_emlrtRTEI, true);
  emxInit_real_T(sp, &k4, 1, &ab_emlrtRTEI, true);
  if (0 <= b_loop_ub - 1) {
    u_size[0] = b_i;
  }
  ...
  for (b_i = 0; b_i < b_loop_ub; b_i++) {
    ...
    if (p_ode_rk_order == 1.0) {
      ...
      i = k4->size[0];
      k4->size[0] = k1->size[0];
      emxEnsureCapacity_real_T(&st, k4, i, &s_emlrtRTEI);
      loop_ub = k1->size[0];
      for (i = 0; i < loop_ub; i++) {
