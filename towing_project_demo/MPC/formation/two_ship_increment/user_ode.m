...
function xdot = user_ode(x, u, p_ode)
    ...
    for i = 1 : Nship 
        ...
        p = x(6*(i-1)+1 : 6*(i-1)+1);
        v = x(6*(i-1)+4 : 6*(i-1)+6);
        df = u(6*(i-1)+1 : 6*(i-1)+3);
        da = u(6*(i-1)+4 : 6*(i-1)+6);
        f_last = u_last(6*(i-1)+1 : 6*(i-1)+3);
        a_last = u_last(6*(i-1)+4 : 6*(i-1)+6);
        f = f_last + df;
        a = a_last + da;
        
        
        R1 = rotate_matrix(fai);
        C1 = C_matrix(M, v);
        B1 = thrusters_configuration(a, thrust_config);
        xdot(6*(i-1)+1 : 6*(i-1)+3, 1) = R1 * v;
        xdot(6*(i-1)+4 : 6*(i-1)+6, 1) = inv(M) * (B1 * f * 1e3 - C1 * v - D * v);
    end
