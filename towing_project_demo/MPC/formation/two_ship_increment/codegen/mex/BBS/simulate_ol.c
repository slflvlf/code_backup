...
static emlrtRSInfo e_emlrtRSI = { 8,   /* lineNo */
  ...
  "C:\\Users\\jiang\\Desktop\\towing_project_demo\\MPC\\formation\\two_ship_increment\\user_control_profile.m"/* pathName */
};
...
void simulate_ol(const emlrtStack *sp, const real_T p[48], real_T p_ode_Nship,
                 ...
                 [12], const real_T p_ode_u_last[12], const real_T p_ode_M[9],
                 const real_T p_ode_D[9], const real_T p_ode_thrust_config[6],
                 real_T p_uparam_nu, real_T p_uparam_Np, const real_T
                 p_uparam_R[11520], emxArray_real_T *tt, emxArray_real_T *xx,
                 emxArray_real_T *uu)
{
  char_T TRANSB1;
  char_T TRANSA1;
  ...
  uu->size[0] = b_loop_ub;
  uu->size[1] = loop_ub;
  emxEnsureCapacity_real_T(&st, uu, i, &p_emlrtRTEI);
  for (i = 0; i < loop_ub; i++) {
  ...
  emxInit_real_T(sp, &r, 2, &w_emlrtRTEI, true);
  ...
  i = tt->size[0];
  tt->size[0] = r->size[1];
  emxEnsureCapacity_real_T(sp, tt, i, &s_emlrtRTEI);
  loop_ub = r->size[1];
  for (i = 0; i < loop_ub; i++) {
  ...
  emxInit_real_T(sp, &k1, 1, &x_emlrtRTEI, true);
  emxInit_real_T(sp, &k2, 1, &y_emlrtRTEI, true);
  emxInit_real_T(sp, &k3, 1, &ab_emlrtRTEI, true);
  emxInit_real_T(sp, &k4, 1, &bb_emlrtRTEI, true);
  if (0 <= b_loop_ub - 1) {
    u_size[0] = b_i;
  }
  ...
  for (b_i = 0; b_i < b_loop_ub; b_i++) {
    ...
    if (p_ode_rk_order == 1.0) {
      ...
      user_ode(&b_st, xplus, u_data, u_size, p_ode_Nship, p_ode_u_last, p_ode_M,
               p_ode_D, p_ode_thrust_config, k4);
      loop_ub = k4->size[0];
      for (i = 0; i < loop_ub; i++) {
        k4->data[i] *= p_ode_tau;
      }
      ...
      user_ode(&b_st, xplus, u_data, u_size, p_ode_Nship, p_ode_u_last, p_ode_M,
               p_ode_D, p_ode_thrust_config, k1);
      loop_ub = k1->size[0];
      for (i = 0; i < loop_ub; i++) {
      ...
      user_ode(&b_st, xplus, u_data, u_size, p_ode_Nship, p_ode_u_last, p_ode_M,
               p_ode_D, p_ode_thrust_config, k2);
      loop_ub = k2->size[0];
      for (i = 0; i < loop_ub; i++) {
        k2->data[i] *= p_ode_tau;
      }
      ...
      i = k4->size[0];
      k4->size[0] = k1->size[0];
      emxEnsureCapacity_real_T(&st, k4, i, &t_emlrtRTEI);
      loop_ub = k1->size[0];
      for (i = 0; i < loop_ub; i++) {
      ...
      user_ode(&b_st, xplus, u_data, u_size, p_ode_Nship, p_ode_u_last, p_ode_M,
               p_ode_D, p_ode_thrust_config, k3);
      i = k4->size[0];
      k4->size[0] = k3->size[0];
