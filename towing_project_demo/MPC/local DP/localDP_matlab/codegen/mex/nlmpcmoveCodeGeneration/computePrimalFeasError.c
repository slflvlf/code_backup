...
real_T computePrimalFeasError(
    const emlrtStack *sp, const real_T x[79], int32_T mLinIneq,
    ...
    int32_T mLB, const real_T lb[79], const int32_T finiteUB_data[],
    int32_T finiteUB_size, int32_T mUB)
{
  ...
  for (idx = 0; idx < mLB; idx++) {
    ...
    if ((finiteLB_data[idx] < 1) || (finiteLB_data[idx] > 79)) {
      emlrtDynamicBoundsCheckR2012b(finiteLB_data[idx], 1, 79, &s_emlrtBCI,
                                    (emlrtCTX)sp);
    }
