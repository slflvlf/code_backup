...
nlobj_plan.Ts = Ts;
nlobj_plan.PredictionHorizon = Np*2;
nlobj.ControlHorizon = Nc;
rng(0)
% 验证nlmpc
validateFcns(nlobj_plan,rand(nx,1),rand(nu,1));

nlobj_plan.MV = struct('Min',{0; 0; 0; -2*pi; -2*pi; -2*pi},...
    'Max',{Fmax; Fmax; Fmax; 2*pi; 2*pi; 2*pi},...
    'RateMin', {-dFmax; -dFmax; -dFmax; -damax; -damax; -damax},...
    'RateMax', {dFmax; dFmax; dFmax; damax; damax; damax});

nlobj_plan.Weights.OutputVariables = [250 250 250 0 0 0]*50;
nlobj_plan.Weights.ManipulatedVariables =[1 1 1 0 0 0];
nlobj_plan.Weights.ManipulatedVariablesRate = [50 50 50 300 300 300]*0;
nlobj_plan.Optimization.CustomCostFcn = "";
