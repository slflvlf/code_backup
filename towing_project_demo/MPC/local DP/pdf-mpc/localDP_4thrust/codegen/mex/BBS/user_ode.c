...
static emlrtRSInfo r_emlrtRSI = { 14,  /* lineNo */
  "user_ode",                          /* fcnName */
  "C:\\Users\\jiang\\Desktop\\towing_project_demo\\MPC\\local DP\\localDP_4thrust\\user_ode.m"/* pathName */
};
...
static emlrtRSInfo v_emlrtRSI = { 42,  /* lineNo */
  "checkcond",                         /* fcnName */
  "C:\\Program Files\\MATLAB\\R2020a\\toolbox\\eml\\lib\\matlab\\matfun\\inv.m"/* pathName */
};
...
static emlrtBCInfo f_emlrtBCI = { -1,  /* iFirst */
  ...
  "u",                                 /* aName */
  "user_ode",                          /* fName */
  "C:\\Users\\jiang\\Desktop\\towing_project_demo\\MPC\\local DP\\localDP_4thrust\\user_ode.m",/* pName */
  0                                    /* checkKind */
};
              ...
              p_ode_D[9], const real_T p_ode_thrust_config[8], real_T xdot[6])
{
  ...
  if (5 > u_size[0]) {
    emlrtDynamicBoundsCheckR2012b(5, 1, u_size[0], &f_emlrtBCI, sp);
  }

  if (6 > u_size[0]) {
    emlrtDynamicBoundsCheckR2012b(6, 1, 5, &f_emlrtBCI, sp);
  }

  if (7 > u_size[0]) {
    emlrtDynamicBoundsCheckR2012b(7, 1, 6, &f_emlrtBCI, sp);
  }

  if (8 > u_size[0]) {
    emlrtDynamicBoundsCheckR2012b(8, 1, 7, &f_emlrtBCI, sp);
  }
  ...
  /*  matrix */
  /*  initialization */
  absx11 = muDoubleScalarCos(u_data[4]);
  a[0] = absx11;
  absx21 = muDoubleScalarSin(u_data[4]);
  a[1] = absx21;
  a[2] = p_ode_thrust_config[0] * absx21 - p_ode_thrust_config[4] * absx11;
  if (*emlrtBreakCheckR2012bFlagVar != 0) {
    emlrtBreakCheckR2012b(sp);
  }

  absx11 = muDoubleScalarCos(u_data[5]);
  a[3] = absx11;
  absx21 = muDoubleScalarSin(u_data[5]);
  a[4] = absx21;
  a[5] = p_ode_thrust_config[1] * absx21 - p_ode_thrust_config[5] * absx11;
  if (*emlrtBreakCheckR2012bFlagVar != 0) {
    emlrtBreakCheckR2012b(sp);
  }

  absx11 = muDoubleScalarCos(u_data[6]);
  a[6] = absx11;
  absx21 = muDoubleScalarSin(u_data[6]);
  a[7] = absx21;
  a[8] = p_ode_thrust_config[2] * absx21 - p_ode_thrust_config[6] * absx11;
  if (*emlrtBreakCheckR2012bFlagVar != 0) {
    emlrtBreakCheckR2012b(sp);
  }

  absx11 = muDoubleScalarCos(u_data[7]);
  a[9] = absx11;
  absx21 = muDoubleScalarSin(u_data[7]);
  a[10] = absx21;
  a[11] = p_ode_thrust_config[3] * absx21 - p_ode_thrust_config[7] * absx11;
  if (*emlrtBreakCheckR2012bFlagVar != 0) {
    emlrtBreakCheckR2012b(sp);
  }
  ...
  b_a[i] = ((1.0 - R_tmp[3] * absx21) - R_tmp[6] * absx11) / R_tmp[0];
  b_a[i + 1] = absx21;
  b_a[i + 2] = absx11;
  absx11 = -R_tmp[5] / R_tmp[8];
  absx21 = (1.0 - R_tmp[7] * absx11) / R_tmp[4];
  b_a[p2] = -(R_tmp[3] * absx21 + R_tmp[6] * absx11) / R_tmp[0];
  b_a[p2 + 1] = absx21;
  b_a[p2 + 2] = absx11;
  absx11 = 1.0 / R_tmp[8];
  absx21 = -R_tmp[7] * absx11 / R_tmp[4];
  b_a[p3] = -(R_tmp[3] * absx21 + R_tmp[6] * absx11) / R_tmp[0];
  b_a[p3 + 1] = absx21;
  b_a[p3 + 2] = absx11;
  ...
  exitg1 = false;
  while ((!exitg1) && (i < 3)) {
    absx11 = (muDoubleScalarAbs(b_a[3 * i]) + muDoubleScalarAbs(b_a[3 * i + 1]))
      + muDoubleScalarAbs(b_a[3 * i + 2]);
    if (muDoubleScalarIsNaN(absx11)) {
      absx31 = rtNaN;
      exitg1 = true;
    } else {
  ...
  if ((absx21 == 0.0) || (absx31 == 0.0) || (absx11 == 0.0)) {
    ...
    if (muDoubleScalarIsNaN(absx11) || (absx11 < 2.2204460492503131E-16)) {
      ...
      emlrt_marshallIn(&d_st, b_sprintf(&d_st, y, emlrt_marshallOut(absx11),
  ...
  absx31 = x[5];
  for (p2 = 0; p2 < 3; p2++) {
    c_a[p2] = ((((a[p2] * u_data[0] + a[p2 + 3] * u_data[1]) + a[p2 + 6] *
                 u_data[2]) + a[p2 + 9] * u_data[3]) * 1000.0 - ((R_tmp[p2] *
      absx11 + R_tmp[p2 + 3] * absx21) + R_tmp[p2 + 6] * absx31)) - ((p_ode_D[p2]
      * absx11 + p_ode_D[p2 + 3] * absx21) + p_ode_D[p2 + 6] * absx31);
  }

  for (p2 = 0; p2 < 3; p2++) {
    xdot[p2 + 3] = (b_a[p2] * c_a[0] + b_a[p2 + 3] * c_a[1]) + b_a[p2 + 6] *
      c_a[2];
  }
