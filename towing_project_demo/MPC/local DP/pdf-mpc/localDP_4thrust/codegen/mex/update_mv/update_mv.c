...
static emlrtRSInfo emlrtRSI = { 27,    /* lineNo */
  "update_mv",                         /* fcnName */
  "C:\\Users\\jiang\\Desktop\\nlmpc\\pdfmpc\\update_mv.m"/* pathName */
};
               ...
               *u_sol, real_T p_sol[32], emxArray_real_T *lesp)
{
  ...
  /* --------------------------------------------------------- */
  ell = (param->Nev - 1.0) / (128.0 * param->Niter);
  if (ell < 0.0) {
  ...
  for (c_i = 0; c_i < i; c_i++) {
    for (sig = 0; sig < loop_ub; sig++) {
      ...
      for (b_i = 0; b_i < 32; b_i++) {
        param->p[b_i] = lesp->data[b_i + 32 * ((int32_T)ind - 1)];
      ...
      if (((int32_T)ell < 1) || ((int32_T)ell > 32)) {
        emlrtDynamicBoundsCheckR2012b((int32_T)ell, 1, 32, &g_emlrtBCI, sp);
      }
      ...
      b_i = (int32_T)ind - 1;
      mv[(int32_T)ell - 1].p = lesp->data[((int32_T)ell + 32 * b_i) - 1];
      st.site = &emlrtRSI;
      ...
      for (i1 = 0; i1 < 32; i1++) {
        c_lesp[i1] = lesp->data[i1 + 32 * b_i];
      }

      for (b_i = 0; b_i < 32; b_i++) {
        lesp->data[b_i + 32 * (int32_T)ind] = c_lesp[b_i];
      }
      ...
      lesp->data[((int32_T)ell + 32 * (b_i - 1)) - 1] = mv[(int32_T)ell - 1].p;
  ...
  for (i = 0; i < 32; i++) {
    p_sol[i] = lesp->data[i + 32 * (lesp->size[1] - 1)];
  }

  loop_ub = lesp->size[1];
  st.site = &b_emlrtRSI;
  user_control_profile(&st, *(real_T (*)[32])&lesp->data[32 * (loop_ub - 1)],
