...
static emlrtRTEInfo p_emlrtRTEI = { 11,/* lineNo */
  7,                                   /* colNo */
  "update_sc",                         /* fName */
  "C:\\Users\\jiang\\Desktop\\nlmpc\\pdfmpc\\update_sc.m"/* pName */
};

static emlrtDCInfo m_emlrtDCI = { 10,  /* lineNo */
  12,                                  /* colNo */
  "update_sc",                         /* fName */
  "C:\\Users\\jiang\\Desktop\\nlmpc\\pdfmpc\\update_sc.m",/* pName */
  1                                    /* checkKind */
...
static emlrtRTEInfo eb_emlrtRTEI = { 10,/* lineNo */
  21,                                  /* colNo */
  "update_sc",                         /* fName */
  "C:\\Users\\jiang\\Desktop\\nlmpc\\pdfmpc\\update_sc.m"/* pName */
};
...
static emlrtRTEInfo hb_emlrtRTEI = { 7,/* lineNo */
  19,                                  /* colNo */
  "update_sc",                         /* fName */
  "C:\\Users\\jiang\\Desktop\\nlmpc\\pdfmpc\\update_sc.m"/* pName */
};
               ...
               real_T Niter)
{
  ...
  BBS(&st, sc_past->p, param->p, param->ell, param->uparam.nu, param->uparam.Np,
      ...
      param->x0, &Jcurrent, &gcurrent);
  if (!(Niter >= 0.0)) {
    emlrtNonNegativeCheckR2012b(Niter, &n_emlrtDCI, sp);
  }

  if (Niter != (int32_T)muDoubleScalarFloor(Niter)) {
    emlrtIntegerCheckR2012b(Niter, &m_emlrtDCI, sp);
  }

  emxInit_real_T(sp, &lesalpha, 1, &hb_emlrtRTEI, true);
  loop_ub = (int32_T)Niter;
  i = lesalpha->size[0];
  lesalpha->size[0] = loop_ub;
  emxEnsureCapacity_real_T(sp, lesalpha, i, &eb_emlrtRTEI);
  for (i = 0; i < loop_ub; i++) {
    lesalpha->data[i] = 0.0;
  }

  emxInit_real_T(sp, &lesp, 1, &gb_emlrtRTEI, true);
  emlrtForLoopVectorCheckR2012b(1.0, 1.0, Niter, mxDOUBLE_CLASS, (int32_T)Niter,
    &p_emlrtRTEI, sp);
  i = lesp->size[0];
  lesp->size[0] = loop_ub;
  emxEnsureCapacity_real_T(sp, lesp, i, &fb_emlrtRTEI);
  for (b_i = 0; b_i < loop_ub; b_i++) {
    i = (int32_T)(b_i + 1U);
    if ((i < 1) || (i > lesp->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i, 1, lesp->size[0], &cb_emlrtBCI, sp);
    }

    lesp->data[i - 1] = sc_past->p;
    if (i > lesalpha->size[0]) {
      emlrtDynamicBoundsCheckR2012b(i, 1, lesalpha->size[0], &db_emlrtBCI, sp);
    }
    ...
    if (sc_past->qgmax <= 0.0) {
      ...
      BBS(&st, sc_past->pJmin, param->p, param->ell, param->uparam.nu,
          ...
          param->ode.u0, param->ode.M, param->ode.D, param->ode.thrust_config,
          &param->ocp, param->x0, &Jcand, &gcand);
      ...
      if (sc_past->ngz == 1.0) {
        ...
        BBS(&st, Jcand_idx_1, param->p, param->ell, param->uparam.nu,
            ...
            param->ode.rk_order, param->ode.u0, param->ode.M, param->ode.D,
            param->ode.thrust_config, &param->ocp, param->x0, &Jcand, &gcand);
      } else {
        vp[0] = sc_past->Z[0];
        vp[1] = sc_past->Z[1];
        vp[2] = muDoubleScalarMin(sc_past->Z[1], muDoubleScalarMax(sc_past->Z[0],
