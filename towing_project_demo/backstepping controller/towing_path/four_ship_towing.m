...
MM1 = M1 + Ma1;
D1 = [MM1(1,1)/100, 0, 0; 0, MM1(2,2)/40, 0; 0, 0, MM1(3,3)/5];
...
global p0_init;
p0_init = zeros(3, 1);
p0_init = [63.15, 4.297, 0]';
p0 = p0_init;
v0 = zeros(3, 1);
R = rotate_matrix(p0);

p1 = p0 + R * delta10;  p2 = p0 + R * delta20;  p3 = p0 + R * delta30;  p4 = p0 + R * delta40;
...
%     eta = [0.9, 0.9, 1.2]';
time_step = 1;
vmax = [4, 4, 1/180*pi/time_step]';%速度饱和项

% load 目的地
Detination = [600, 50, 30*deg2rad]';


% leader guidance pid 参数
ei_leader = zeros(3, 1);
pd0_ref = p0_init;
kp_ref = diag([0.8*5, 0.8*5, 0.8*5])*0.05;
...
%     [pd0, dot_pd0, ddot_pd0] = ref_model(T, t, p0_init, Detination);
    ...
    % 非直路径
    index=findNearestPoint(p0(1:2),r(1:2,:));   pd0 = r(:, index);  ddot_pd0 = zeros(3, 1);
    a = 0.15;   b = 0.005;
    if i == 1
        pd0_hat = pd0;  dot_pd0_hat = zeros(3, 1);
    else
        [pd0_hat, dot_pd0_hat] = alpha_beta_observer1(a, b, pd0, 1, pd0_hat, dot_pd0_hat);
    end

    
    [pd0_ref, dot_pd0_ref, ddot_pd0_ref, ei_leader] = leader_guiddance_pid_controller(p0, v0, pd0_hat, dot_pd0_hat, pd0_ref,...
...
%     tug4_semi_motion_display(p0, p1, p2, p3, p4, 1);
...
plot31_3lines(pd0_his, pd0_ref_his, p0_his, '拖航循迹效果', {"pd0", 'pd0_{ref}', 'p0'}, 6, true)
