...
delta10 = [375-200, 150, 0*deg2rad]';
delta20 = [-375+200, 150, 0*deg2rad]';
delta30 = [-375+200, -150, 0*deg2rad]';
delta40 = [375-200, -150, 0*deg2rad]';
...
%     [dot_pd1, dot_pd2, dot_pd3, dot_pd4] = deal(zeros(3, 1));
    ...
    v1_his(:, i) = v1;
    pd1_his(:, i) = pd1;
    vd1_his(:, i) = rotate_matrix(pd1)' * dot_pd1;
    
    p2_his(:, i) = p2;
    v2_his(:, i) = v2;
    pd2_his(:, i) = pd2;
    vd2_his(:, i) = rotate_matrix(pd2)' * dot_pd2;
    
    p3_his(:, i) = p3;
    v3_his(:, i) = v3;
    pd3_his(:, i) = pd3;
    vd3_his(:, i) = rotate_matrix(pd3)' *dot_pd3;
    
    p4_his(:, i) = p4;
    v4_his(:, i) = v4;
    pd4_his(:, i) = pd4;
    vd4_his(:, i) = rotate_matrix(pd4)' *dot_pd4;
...
plot(pd1_his(1, :), pd1_his(2, :)); hold on; plot(p1_his(1, :), p1_his(2,:)); hold on; 
plot(pd2_his(1, :), pd2_his(2, :)); hold on; plot(p2_his(1, :), p2_his(2,:)); hold on; 
plot(pd3_his(1, :), pd3_his(2, :)); hold on; plot(p3_his(1, :), p3_his(2,:)); hold on; 
plot(pd4_his(1, :), pd4_his(2, :)); hold on; plot(p4_his(1, :), p4_his(2,:)); hold off;
