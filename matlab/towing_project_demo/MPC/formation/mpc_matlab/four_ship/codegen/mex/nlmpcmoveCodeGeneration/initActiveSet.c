...
void initActiveSet(const emlrtStack *sp, h_struct_T *obj)
{
  ...
  for (colOffsetATw = idxFillStart; colOffsetATw <= b; colOffsetATw++) {
    ...
    if ((colOffsetATw < 1) || (colOffsetATw > i)) {
      emlrtDynamicBoundsCheckR2012b(colOffsetATw, 1, i, &le_emlrtBCI,
                                    (emlrtCTX)sp);
    }
  ...
  obj->nWConstr[3] = 0;
  obj->nWConstr[4] = 0;
  obj->nActiveConstr = obj->nWConstr[0] + 192;
  ...
  for (idx_local = 0; idx_local <= nWFixed; idx_local++) {
    ...
    if ((idx_local + 1 < 1) || (idx_local + 1 > i)) {
      emlrtDynamicBoundsCheckR2012b(idx_local + 1, 1, i, &le_emlrtBCI,
                                    (emlrtCTX)sp);
    }
  ...
  st.site = &tc_emlrtRSI;
  for (idx_local = 0; idx_local < 192; idx_local++) {
    idxFillStart = (nWFixed + idx_local) + 2;
    i = obj->Wid.size[0];
    if ((idxFillStart < 1) || (idxFillStart > i)) {
      emlrtDynamicBoundsCheckR2012b(idxFillStart, 1, i, &le_emlrtBCI,
                                    (emlrtCTX)sp);
    }
