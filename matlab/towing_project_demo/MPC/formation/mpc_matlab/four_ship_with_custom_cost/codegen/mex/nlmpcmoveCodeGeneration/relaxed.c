             ...
             f_struct_T *QPObjective, d_struct_T *qpoptions)
{
  ...
  for (idx = 0; idx <= nVarOrig - 2; idx++) {
    if ((idx + 1 < 1) || (idx + 1 > 265)) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, 265, &vd_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    beta += Hessian[idx + 265 * idx];
  }
  ...
  mFiniteLB = WorkingSet->sizes[3] - 384;
  ...
  for (idx = 0; idx < 192; idx++) {
    ...
    b_st.site = &qh_emlrtRSI;
    if ((tmp_tmp_tmp + 192 < 1) || (tmp_tmp_tmp + 192 > i1)) {
      emlrtDynamicBoundsCheckR2012b(tmp_tmp_tmp + 192, 1, i1, &vc_emlrtBCI,
                                    &b_st);
    }
    b_tf = WorkingSet->isActiveConstr.data[tmp_tmp_tmp + 191];
    memspace->workspace_int.data[idx] = tf;
    memspace->workspace_int.data[idx + 192] = b_tf;
  ...
  for (idx = 0; idx <= temp + 1; idx++) {
    ...
    if ((idx + 385 < 1) || (idx + 385 > i)) {
      emlrtDynamicBoundsCheckR2012b(idx + 385, 1, i, &wd_emlrtBCI, &st);
    }
    memspace->workspace_int.data[idx + 384] = tf;
  ...
  if (TrialState->state != -6) {
    ...
    for (idx = 0; idx < 192; idx++) {
      if ((memspace->workspace_int.data[idx] != 0) &&
          (memspace->workspace_int.data[idx + 192] != 0)) {
        tf = true;
      } else {
        tf = false;
      }
    ...
    for (idx = temp; idx <= mFiniteLB; idx++) {
      ...
      if (WorkingSet->Wid.data[idx - 1] == 3) {
        ...
        if ((i1 + 384 < 1) || (i1 + 384 > i)) {
          emlrtDynamicBoundsCheckR2012b(i1 + 384, 1, i, &ud_emlrtBCI,
                                        (emlrtCTX)sp);
        }
        i = TrialState->lambda.size[0];
        if (idx > i) {
        ...
        TrialState->lambda.data[idx - 1] *=
            (real_T)memspace->workspace_int.data[i1 + 383];
      }
    }
  }
  st.site = &ph_emlrtRSI;
  temp = WorkingSet->sizes[0];
  mFiniteLB = (WorkingSet->sizes[3] - WorkingSet->sizes[2]) - 384;
  idx = WorkingSet->nActiveConstr;
  while ((idx > temp + 192) && (nActiveLBArtificial > 0)) {
    i = WorkingSet->Wid.size[0];
    if ((idx < 1) || (idx > i)) {
