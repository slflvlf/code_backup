                ...
                int32_T *exitflag)
{
  ...
  *exitflag = 1;
  ...
  do {
    exitg1 = 0;
    if (TrialState->FunctionEvaluations < 26500) {
      if ((*evalWellDefined) &&
          (phi_alpha <=
        ...
        *alpha *= 0.7;
        ...
        if (tooSmallX) {
          *exitflag = -2;
          ...
          st.site = &ci_emlrtRSI;
          for (loop_ub = 0; loop_ub < WorkingSet_nVar; loop_ub++) {
            if ((loop_ub + 1 < 1) || (loop_ub + 1 > 265)) {
              emlrtDynamicBoundsCheckR2012b(loop_ub + 1, 1, 265, &ee_emlrtBCI,
                                            (emlrtCTX)sp);
            }
          ...
          *evalWellDefined = (y_size == 1);
          ...
          if (*evalWellDefined) {
            phi_alpha = 0.0;
            for (y_size = 0; y_size < 192; y_size++) {
