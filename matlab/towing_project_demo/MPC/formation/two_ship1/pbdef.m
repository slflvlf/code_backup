...
u20 = [120, 60, 60, 0*pi/180, 180*pi/180, 180*pi/180]';
% u10 = [100, 50, 50, 180*pi/180, 0*pi/180, 0*pi/180]';
% u20 = [100, 50, 50, 180*pi/180, 0*pi/180, 0*pi/180]';
p_ode.u0 = [u10; u20];
...
% Definition of p_ocp
%-------------------------------------------------------------------------------
p_ocp.Q = diag(repmat([1e8;1e8;5e10;1;1;1], p_ode.Nship, 1));%多条船的增益系数一样
p_ocp.R = diag(repmat([1e5; 1e5; 1e5; 0; 0; 0]*1e0*1, p_ode.Nship, 1));
p_ocp.Rdu = diag(repmat([1; 1; 1; 1; 1; 1]*1e0*1, p_ode.Nship, 1));
...
f1_his(1, :) = p_ode.u0(1:3)';  a1_his(1, :) = p_ode.u0(4:6)';
f2_his(1, :) = p_ode.u0(7:9)';  a2_his(1, :) = p_ode.u0(10:12)';
...
%         param.ocp.xd_his(j, :) = [pd1; vd1; pd2; vd2]';        
        param.ocp.xd_his(j, :) = [pd0; vd0; pd0; vd0]';          
    end
...
%     x_hat = [p1_hat; v1_hat; p2_hat; v2_hat];
    ...
    current_time = i;
    [tau1_wind_tug, tau1_current_tug, tau1_wave1_tug, tau1_wave2_tug] = tau_env_tug_interface(p1_his(i, :)', ...
        current_time, wind_param, current_param, wave_param);
    [tau2_wind_tug, tau2_current_tug, tau2_wave1_tug, tau2_wave2_tug] = tau_env_tug_interface(p2_his(i, :)', ...
        current_time, wind_param, current_param, wave_param);
