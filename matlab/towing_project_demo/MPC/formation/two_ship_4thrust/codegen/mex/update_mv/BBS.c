         ...
         real_T *J, real_T *g)
{
  ...
  /* --------------------------------------------------------- */
  memcpy(&p[0], &param_p[0], 64U * sizeof(real_T));
  ...
  i = (int32_T)param_ell;
  if ((i < 1) || (i > 64)) {
    emlrtDynamicBoundsCheckR2012b(i, 1, 64, &k_emlrtBCI, sp);
  }
  ...
  simulate_ol(&st, p, param_ode_Nship, param_ode_tau, param_ode_rk_order,
              param_ode_M, param_ode_D, param_ode_thrust_config, param_x0,
  ...
  user_ocp(&st, xx, uu, param_ode_Nship, param_ode_u0, param_uparam_Np,
           param_ocp->Q, param_ocp->R, param_ocp->Rdu, param_ocp->rd,
           param_ocp->dF_max, param_ocp->da_max, J, g);
