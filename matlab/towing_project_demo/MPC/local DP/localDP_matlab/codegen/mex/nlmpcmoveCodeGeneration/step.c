...
boolean_T step(c_nlmpcmoveCodeGenerationStackD *SD, const emlrtStack *sp,
               int32_T *STEP_TYPE, real_T Hessian[6241], const real_T lb[79],
  ...
  do {
    ...
    if (guard1) {
      if (TrialState->state != -6) {
        ...
        for (tmp_size = 0; tmp_size < 79; tmp_size++) {
          iH0 = 79 * tmp_size;
          b_st.site = &vh_emlrtRSI;
          g_xcopy(&b_st, tmp_size, Hessian, iH0 + 1);
          Hessian[tmp_size + 79 * tmp_size] = nrmGradInf;
          iH0 += tmp_size;
          b_st.site = &vh_emlrtRSI;
          g_xcopy(&b_st, 78 - tmp_size, Hessian, iH0 + 2);
        }
      }
    }
  } while (exitg1 == 0);
