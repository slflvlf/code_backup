...
void modifyOverheadRegularized_(const emlrtStack *sp, h_struct_T *obj)
{
  ...
  b_st.prev = &st;
  b_st.tls = st.tls;
  mIneq_tmp = obj->sizes[2] + 79;
  offsetEq2 = obj->sizes[2] + 139;
  b = obj->sizes[0];
  st.site = &vc_emlrtRSI;
  ...
  for (idx_col = 0; idx_col < b; idx_col++) {
    ...
    for (idx_row = 80; idx_row <= b_b; idx_row++) {
      i = obj->ATwset.size[0];
  ...
  for (idx_col = 0; idx_col <= mIneq_tmp - 80; idx_col++) {
    idx_lb = idx_col + 79;
    colOffsetAineq = obj->ldA * idx_col;
    st.site = &vc_emlrtRSI;
    if (idx_col + 79 > 2147483646) {
      b_st.site = &db_emlrtRSI;
      check_forloop_overflow_error(&b_st);
    }
    for (idx_row = 80; idx_row <= idx_lb; idx_row++) {
      i = obj->Aineq.size[0];
    ...
    b = obj->nVar;
    st.site = &vc_emlrtRSI;
    if ((idx_col + 81 <= obj->nVar) && (obj->nVar > 2147483646)) {
      b_st.site = &db_emlrtRSI;
      check_forloop_overflow_error(&b_st);
    }
  ...
  for (idx_col = 0; idx_col < 60; idx_col++) {
    ...
    for (idx_row = 80; idx_row <= mIneq_tmp; idx_row++) {
      i = obj->Aeq.size[0];
      i1 = idx_row + colOffsetAineq;
      if ((i1 < 1) || (i1 > i)) {
        emlrtDynamicBoundsCheckR2012b(i1, 1, i, &nb_emlrtBCI, (emlrtCTX)sp);
  ...
  b = obj->sizes[2] + 199;
  st.site = &vc_emlrtRSI;
  if (obj->sizes[2] + 199 > 2147483646) {
    b_st.site = &db_emlrtRSI;
    check_forloop_overflow_error(&b_st);
  }
  for (colOffsetAineq = 80; colOffsetAineq <= b; colOffsetAineq++) {
    i = obj->lb.size[0];
    if (colOffsetAineq > i) {
      emlrtDynamicBoundsCheckR2012b(colOffsetAineq, 1, i, &nb_emlrtBCI,
                                    (emlrtCTX)sp);
  ...
  for (idx_col = idx_lb; idx_col <= b; idx_col++) {
    ...
    if (obj->Wid.data[idx_col - 1] == 3) {
      ...
      idx_local = colOffsetAineq + 80;
      b_tmp = colOffsetAineq + 78;
      st.site = &vc_emlrtRSI;
      if (colOffsetAineq + 78 > 2147483646) {
        b_st.site = &db_emlrtRSI;
        check_forloop_overflow_error(&b_st);
      }
      for (idx_row = 80; idx_row <= b_tmp; idx_row++) {
        i = obj->ATwset.size[0];
        i1 = idx_row + colOffsetATw;
        if ((i1 < 1) || (i1 > i)) {
          emlrtDynamicBoundsCheckR2012b(i1, 1, i, &nb_emlrtBCI, (emlrtCTX)sp);
      ...
      i = obj->ATwset.size[0];
      i1 = (colOffsetAineq + colOffsetATw) + 79;
      if ((i1 < 1) || (i1 > i)) {
        emlrtDynamicBoundsCheckR2012b(i1, 1, i, &nb_emlrtBCI, (emlrtCTX)sp);
      }
      obj->ATwset.data[i1 - 1] = -1.0;
      b_b = obj->nVar;
      st.site = &vc_emlrtRSI;
      if ((colOffsetAineq + 80 <= obj->nVar) && (obj->nVar > 2147483646)) {
        b_st.site = &db_emlrtRSI;
        check_forloop_overflow_error(&b_st);
      }
