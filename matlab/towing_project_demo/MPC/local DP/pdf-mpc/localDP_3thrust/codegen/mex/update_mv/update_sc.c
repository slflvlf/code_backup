               ...
               real_T Niter)
{
  ...
  BBS(&st, sc_past->p, param->p, param->ell, param->uparam.nu, param->uparam.Np,
      param->uparam.R, param->ode.tau, param->ode.rk_order, param->ode.M,
      param->ode.D, param->ode.thrust_config, &param->ocp, param->x0, &Jcurrent,
      &gcurrent);
  if (!(Niter >= 0.0)) {
  ...
  for (b_i = 0; b_i < loop_ub; b_i++) {
    ...
    if ((i < 1) || (i > lesp->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i, 1, lesp->size[0], &bb_emlrtBCI, sp);
    }

    lesp->data[i - 1] = sc_past->p;
    if (i > lesalpha->size[0]) {
      emlrtDynamicBoundsCheckR2012b(i, 1, lesalpha->size[0], &cb_emlrtBCI, sp);
    }
