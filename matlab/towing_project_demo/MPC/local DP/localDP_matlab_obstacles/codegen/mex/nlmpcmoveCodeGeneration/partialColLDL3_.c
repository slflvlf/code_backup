                     ...
                     int32_T NColsRemain)
{
  ...
  for (k = 0; k < 48; k++) {
    ...
    for (idx = 0; idx <= subRows; idx++) {
      ...
      if ((lda < 1) || (lda > i)) {
        emlrtDynamicBoundsCheckR2012b(lda, 1, i, &ve_emlrtBCI, (emlrtCTX)sp);
      }
    ...
    for (idx = 0; idx <= b; idx++) {
      i = offsetColK + idx;
      if ((i < 1) || (i > 1)) {
        emlrtDynamicBoundsCheckR2012b(i, 1, 1, &ue_emlrtBCI, (emlrtCTX)sp);
      }
      if ((idx + 1 < 1) || (idx + 1 > 1)) {
    ...
    for (idx = 0; idx <= b; idx++) {
      if ((idx + 1 < 1) || (idx + 1 > 1)) {
        emlrtDynamicBoundsCheckR2012b(idx + 1, 1, 1, &ue_emlrtBCI,
                                      (emlrtCTX)sp);
      }
      i = offsetColK + idx;
      if ((i < 1) || (i > 1)) {
    ...
    if ((LD_diagOffset < 1) || (LD_diagOffset > i)) {
      emlrtDynamicBoundsCheckR2012b(LD_diagOffset, 1, i, &ve_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    ...
    if (LD_diagOffset_tmp < NColsRemain) {
      ...
      for (idx = 0; idx < 48; idx++) {
        ...
        for (LD_diagOffset_tmp = 0; LD_diagOffset_tmp <= i;
          ...
          if ((lda < 1) || (lda > 1)) {
            emlrtDynamicBoundsCheckR2012b(lda, 1, 1, &te_emlrtBCI,
                                          (emlrtCTX)sp);
          }
          lda = obj->FMat.size[0] * obj->FMat.size[1];
          subRows = offsetColK + LD_diagOffset_tmp;
          if ((subRows < 1) || (subRows > lda)) {
            emlrtDynamicBoundsCheckR2012b(subRows, 1, lda, &ve_emlrtBCI,
                                          (emlrtCTX)sp);
          }
