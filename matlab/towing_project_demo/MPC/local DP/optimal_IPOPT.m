...
%         September 18, 2008
function [x, info] = optimal_IPOPT(p)
  ...
  % The callback functions.这些函数是必须的
  funcs.objective         = @(x)objective(x, p);
  funcs.constraints       = @constraints;
  funcs.gradient          = @(x)gradient(x, p);
...
function f = objective (x, p)
  f = (x(1) - x(2))^2 + ...
      (x(2) + x(3) - 2)^2 + ...
      (x(4) - 1)^2 + (x(5) - 1)^2 - 2*p^2*x(1);

% ----------------------------------------------------------------------
function g = gradient (x, p)
  g = 2*[ x(1) - x(2)-p^2;
	  x(2) + x(3) - 2 - x(1) + x(2);
	  x(2) + x(3) - 2;
	  x(4) - 1;
